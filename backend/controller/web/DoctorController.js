...
module.exports = {
  ...
  add: async (req, res, next) => {
    ...
    try {
      const oldDoctors = await DoctorsModel.findOne({ name: name, phoneNo: phoneNo });
      if (oldDoctors) {
        res.status(400).send('Doctor already exists');
      } else {
        const doctor = new DoctorsModel({ name: name, phoneNo: phoneNo });
        await doctor.save();
  ...
  update: async (req, res, next) => {
    ...
    const { name, phoneNo } = req.body;
    try {
      await DoctorsModel.findByIdAndUpdate(id, { name: name, phoneNo: phoneNo });
